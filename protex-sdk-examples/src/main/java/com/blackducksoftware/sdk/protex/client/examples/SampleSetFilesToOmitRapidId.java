 ...
 * - How to omit files from Rapid ID
 * - How to determine if a file is omitted from Rapid ID
 */
public class SampleSetFilesToOmitRapidId extends BDProtexSample {
    ...
    private static void usage() {
        String className = SampleSetFilesToOmitRapidId.class.getSimpleName();
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            // Filter nodes to files only - faster to manually filter than call the API again due to network
            // communication time
            List<CodeTreeNode> fileNodes = new ArrayList<CodeTreeNode>();
            List<CodeTreeNode> fileNodesNoHtml = new ArrayList<CodeTreeNode>();

            // Choose to not get any html files - this is to help demonstrate the behavior of the omit status method
            for (CodeTreeNode node : nodes) {
                if (CodeTreeNodeType.FILE.equals(node.getNodeType())) {
                    if (!node.getName().endsWith("html")) {
                        fileNodesNoHtml.add(node);
                    }
                    fileNodes.add(node);
                }
            }

            // Set all files to omit rapid ID. Passing in non-existant files or folder paths has no effect
            projectApi.setOmitFromRapidId(projectId, fileNodesNoHtml, true);

            // Ask for the omit status of all files - note that all files nodes are being passed in
            List<CodeTreeNode> omittedNodes = projectApi.getFilesOmittedFromRapidId(projectId, fileNodes);

            // The list of omitted nodes matches the list of nodes passed into the set above - non-omitted nodes have
            // been filtered
            System.out.println(fileNodes.size() + " total files, " + omittedNodes.size() + " omitted out of " + fileNodesNoHtml.size()
                    + " requested for omission");
        } catch (Exception e) {
