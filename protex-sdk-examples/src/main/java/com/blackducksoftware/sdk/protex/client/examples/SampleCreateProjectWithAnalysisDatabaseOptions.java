 ...
 * - How to set analysis settings on the project
 */
public class SampleCreateProjectWithAnalysisDatabaseOptions extends BDProtexSample {
    ...
    private static void usage() {
        String className = SampleCreateProjectWithAnalysisDatabaseOptions.class.getSimpleName();
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            try {
                ...
                AnalysisDatabaseOptions analysisOptions = projectApi.getAnalysisDatabaseOptions(projectId);

                // Set to only use the standard BlackDuck code match and string searches, if the settings are not forced
                if (!analysisOptions.isForceCodeMatchOptions()) {
                    analysisOptions.setCustomCodeMatchesOption(false);
                    analysisOptions.setStandardCodeMatchesOption(true);
                } else if (!analysisOptions.isForceStringSearchOptions()) {
                    analysisOptions.setCustomStringSearchesOption(false);
                    analysisOptions.setStandardStringSearchesOption(true);
                }

                projectApi.updateAnalysisDatabaseOptions(projectId, analysisOptions);
            } catch (SdkFault e) {
                System.err.println("createProject() failed: " + e.getMessage());
                throw new RuntimeException(e);
            }
