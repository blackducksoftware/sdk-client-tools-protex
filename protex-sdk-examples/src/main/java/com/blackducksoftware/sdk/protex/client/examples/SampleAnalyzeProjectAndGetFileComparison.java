 ...
 */
public class SampleAnalyzeProjectAndGetFileComparison extends BDProtexSample {
    ...
    private static void usage() {
        String className = SampleAnalyzeProjectAndGetFileComparison.class.getSimpleName();
        ...
        try {
            ...
            if (filesWithMatches.isEmpty()) {
                ...
                for (CodeMatchDiscovery discovery : discoveries) {
                    ...
                    if (matchedType != null) {
                        ProtexFile projectFile = new ProtexFile();
                        projectFile.setComponentOrProjectId(projectId);
                        projectFile.setFilePath(discovery.getFilePath());
                        projectFile.setFileType(ProtexFileSourceType.PROJECT);

                        ProtexFile matchedFile = new ProtexFile();
                        matchedFile.setComponentOrProjectId(comparisonKey);
                        matchedFile.setFilePath(discovery.getMatchingFileLocation().getFilePath());
                        matchedFile.setFileType(matchedType);

                        try {
                            List<RelatedSnippets> similarities = fileComparisonApi.getFileSimilarities(projectFile, matchedFile);
                            List<RelatedSnippets> differences = fileComparisonApi.getFileDifferences(projectFile, matchedFile);
                            ...
                            if (ErrorCode.PROTEX_FILE_SOURCE_NOT_AVAILABLE.equals(fault.getFaultInfo().getErrorCode())) {
                                System.out.println("Source not available for component path: " + matchedFile.getComponentOrProjectId() + ":"
                                        + matchedFile.getFilePath());
                            } else {
                                System.err.println("Error reading sim/diff data for " + projectFile.getFilePath() + " to "
                                        + matchedFile.getComponentOrProjectId()
                                        + ":" + matchedFile.getFilePath());
