 ...
 * 
 * It demonstrates: - How to create SDK client programs that run multi threaded
 * with a configurable thread pool - How to assign a user to a project - How to
 * remove a user from a project - How to handle errors in the multi threaded
 * environment - How to speed up tasks that would take quite long if executed
 * sequentially (See measurements below and discussion)
 * 
 ...
 *         test-1@example.com,test
 *         -2@example.com,test-3@example.com,test-4@example.
 *         com,test-5@example.com,test-1@example.com:
 * 
 *         Clear time (1 threads): 17807 (60 calls :: 0)
 *         Assign time (1threads): 8792 (60 calls :: 10)
 * 
 *         Clear time (5 threads): 5904 (60 calls :: 0)
 *         Assign time (5 threads): 2140 (60 calls :: 10)
 * 
 *         Clear time (10 threads): 5923 (60 calls :: 0)
 *         Assign time (10 threads): 1567 (60 calls :: 10)
 * 
 *         Clear time (20 threads): 14286 (60 calls :: 0)
 *         Assign time (20 threads): 5706 (60 calls :: 10)
 * 
 *         Yes, the duplicate of assigning test-1@example.com is intentional to
 *         test error conditions as well.
 * 
 *         As we see and increasing thread pool does only improve run time
 ...
 *         salt)
 */
public class SampleAssignProjectUsersNToMX extends BDProtexSample {
        ...
        for (String projectId : projectIds) {
            for (String userId : userIds) {
                Callable<SdkFault> worker = new AssignProjectUser(projectId,
                        userId);
                System.out
                .println("Assigning: " + userId + " ==> " + projectId);
    ...
    private static void usage() {
        String className = SampleAssignProjectUsersNToMX.class.getSimpleName();
    ...
    public static void main(String[] args) throws Exception {
        ...
        SampleAssignProjectUsersNToMX myInstance = new SampleAssignProjectUsersNToMX();
